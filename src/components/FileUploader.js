    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Process each sheet
          const clientData = XLSX.utils.sheet_to_json(
            workbook.Sheets["Clients 1"]
          );
          const workerData = XLSX.utils.sheet_to_json(
            workbook.Sheets["Worker 1"]
          );
  ...
  const processWorkerData = (data) => {
    return data.map((worker) => ({
      ...
      Skills: worker.Skills
        ? worker.Skills.split(",").map((skill) => skill.trim())
        : [],
  ...
  const processTaskData = (data) => {
    return data.map((task) => ({
      ...
      RequiredSkills: task.RequiredSkills
        ? task.RequiredSkills.split(",").map((skill) => skill.trim())
        : [],
