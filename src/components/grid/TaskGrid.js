  ...
  const [columnDefs] = useState([
    ...
    { 
      ...
      valueSetter: params => {
        ...
        params.data.RequiredSkills = params.newValue.split(',').map(skill => skill.trim());
        return true;
      }
    },
    { 
      ...
      valueSetter: params => {
        try {
          ...
          if (params.newValue.includes('-')) {
            const [start, end] = params.newValue.split('-').map(num => parseInt(num.trim()));
            params.data.PreferredPhases = Array.from({ length: end - start + 1 }, (_, i) => start + i);
            return true;
          }
          
          // Handle comma-separated values
          params.data.PreferredPhases = params.newValue.split(',').map(phase => parseInt(phase.trim()));
          return true;
        } catch (error) {
